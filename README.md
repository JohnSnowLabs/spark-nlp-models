...
| Explain Clinical Document (type-4)     | `explain_clinical_doc_ade`  | `2.6.2` |   `en` | a pipeline for `Adverse Drug Events (ADE)` with `ner_ade_biobert`, and `classifierdl_ade_conversational_biobert`. It will extract `ADE` and `DRUG` clinical entities, and then assign ADE status to a text(`Negative` means ADE, `Neutral` means not related to ADE).        | [Download](https://s3.amazonaws.com/auxdata.johnsnowlabs.com/clinical/models/explain_clinical_doc_ade_en_2.6.0_2.4_1601455031009.zip)  |
